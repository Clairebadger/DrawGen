import type { FormationInfo, Round, Draw } from "../constants/VFSContstants";
import {
    vfs2WayAdvancedHS,
...
let getDraw = (draw: DrawType): Draw => {
...
let getRoundPoints = (round: Round): number => {
    return round.reduce((acc, formation) => {
        let lastChar = formation.name[formation.name.length - 1];
...
let createVfsDraw = (draw: DrawType, numFlyers: number): Draw => {
    let pool: FormationInfo[] | undefined;
    if (numFlyers === 2) {
...
let createRoundsVfs4Way = (pool: Round, rounds: number): Draw => {
    let maxPoints = 5;
    let draw: Draw = [];
    let poolCopy: Round = JSON.parse(JSON.stringify(pool));
    ...
    for (let i = 0; i < rounds; i++) {
        ...
        while (getRoundPoints(draw[i]) < maxPoints) {
            ...
            draw[i].push(formation);
        }
    }
    return draw;
}

let createRoundsVfs2Way = (hspool: Round, lspool: Round, rounds: number): Draw => {
    let maxPoints = hspool.length < 21 ? 4 : 5;
    let draw: Draw = [];
    let hsPoolCopy: Round = JSON.parse(JSON.stringify(hspool));
    let lsPoolCopy: Round = JSON.parse(JSON.stringify(lspool));
